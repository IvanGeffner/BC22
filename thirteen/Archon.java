...
public class Archon extends Robot {
    ...
    final static int MINER_WAITING_TURNS = 40;
    final static int MAX_SOLDIER_DIFF = 4;
    ...
    void buildUnit(){
        ...
        if (r.t != null){
            ...
            if (constructRobotGreedy(r.t, target)){
                if (r.t == RobotType.MINER) minerRound = rc.getRoundNum();
                comm.cancelFirstReservation();
            }
            return;
        }
        ...
        if (closestEnemy == null || minerScore < 0) {
            ...
            if (rc.getTeamLeadAmount(rc.getTeam()) >= Constants.MIN_LEAD_STOP_MINERS) {
                ...
                if ((explore.cumulativeLead > 50 && rc.getRoundNum() - minerRound > MINER_WAITING_TURNS) || minerScore <= soldierScore) {
    ...
    void computeClosestEnemy(){
        ...
        for (RobotInfo ally : allies){
            if (!Util.isAttacker(ally.getType())) continue;
            --soldierDiff;
        }
    }
    ...
    boolean endangered(){
        ...
        if (soldierDiff < 0) return false;
        if (comm.getArchonCount() <= 1) return true;
        if (soldierDiff >= MAX_SOLDIER_DIFF) return false;
        return true;
    }
