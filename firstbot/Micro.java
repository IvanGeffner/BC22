...
public class Micro {
    ...
    static double myDPS;
    static final int MAX_MICRO_BYTECODE = 3000;
    ...
    boolean doMicro(){
        ...
        /*if (rc.getType().ordinal() >= 2) {
            units = rc.senseNearbyRobots(18, rc.getTeam()); //TODO: weird radius?
            for (RobotInfo unit : units) {
                if (rc.getEnergyUsed() > MAX_MICRO_BYTECODE) break;
    ...
    class MicroInfo{
        ...
        public MicroInfo(Direction dir){
            ...
            if (!rc.canMove(dir)) canMove = false;
                ...
                if (!hurt){
                    ...
                    //this.alliesTargeting += DPS[myType];
        ...
        /*void updateAlly(UnitInfo unit){
            if (!canMove || spider) return;
            if (uc.isObstructed(location, unit.getLocation())) return;
            int dist = unit.getLocation().distanceSquared(location);
            int type = unit.getType().ordinal();
            if (unit.getCocoonTurns() <= 1 && dist <= 8) alliesTargeting += DPS[type];
        }*/
