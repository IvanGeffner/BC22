...
public class Communication {
    ...
    int getSavedLead(){
        ...
        try {
            ...
            for (; i != finalElement; ++i){
                int code = rc.readSharedArray(RESERVE_LEAD_INDEX + (i%LEAD_QUEUE_SIZE));
                ans += rTypes[code&7].getLeadWorth(0);
            }
        } catch (Exception e){
            e.printStackTrace();
        }
    ...
    Reservation getReservedRobot(){
        ...
        try {
            ...
            for (; i != finalElement; ++i){
                int code = rc.readSharedArray(RESERVE_LEAD_INDEX + (i%LEAD_QUEUE_SIZE));
                if ((code >>> 3) != myArchonIndex) r.savedLead += rTypes[code&7].getLeadWorth(0);
                else {
                    r.t = rTypes[code & 7];
                    return r;
                }
